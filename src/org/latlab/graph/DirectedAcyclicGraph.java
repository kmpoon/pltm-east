/**
 * DirectedAcyclicGraph.java Copyright (C) 2006 Tao Chen, Kin Man Poon, Yi Wang,
 * and Nevin L. Zhang
 */
...
public class DirectedAcyclicGraph extends AbstractGraph {
    ...
    /**
     * Copy constructor. The nodes and edges of this graph are added according
     * to those of the specified graph. However, those nodes added are not the
     * same instances as those of the specified graph, but only the names of
     * them are the same.
     * 
     * @param graph
     *            graph to copy from
    ...
    public DirectedAcyclicGraph(DirectedAcyclicGraph graph) {
        ...
        for (Edge edge : graph._edges) {
            addEdge(getNode(edge.getHead().getName()), getNode(edge.getTail()
                .getName()));
        }
    }
    ...
    public final UndirectedGraph computeMoralGraph() {
        ...
        for (Edge edge : _edges) {
            moralGraph.addEdge(
                moralGraph.getNode(edge.getHead().getName()), moralGraph
        ...
        for (AbstractNode node : _nodes) {
            ...
            for (DirectedNode parent1 : dNode.getParents()) {
                ...
                for (DirectedNode parent2 : dNode.getParents()) {
                    AbstractNode neighbor2 =
                        moralGraph.getNode(parent2.getName());
     ...
     */
    @Override
    public final int depthFirstSearch(
     ...
     *            second vertex
     */
    public void removeEdge(DirectedNode vertex1, DirectedNode vertex2) {
        removeEdge(vertex1.getEdge(vertex2));
    }
    ...
    /**
     * Reverses the directions of the edges and returns the resulting directed
     * graph.
     * 
    ...
    /**
     * Returns an undirected graph based on this graph by ignoring the
    ...
    /**
     * Copies the nodes and edges from this graph to the given graph. The nodes
     * and edges in the copied graph are separate instances from those in this
     * graph.
     * 
     * @param graph
     *            graph copied to and returned
