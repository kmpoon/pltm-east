...
public class FunctionIterator {
	 ...
	 */
	public FunctionIterator(Function function, List<DiscreteVariable> order) {
	...
	private void iterate(
		Visitor visitor, int[] states, int[] statesInOriginalOrder,
		int currentIteratingDimension) {
		if (currentIteratingDimension == order.size()) {
			visit(visitor, states, statesInOriginalOrder);
		} else {
			DiscreteVariable currentIteratingVariable =
				order.get(currentIteratingDimension);
			for (int state = 0; state < currentIteratingVariable.getCardinality(); state++) {
				...
				iterate(
					visitor, states, statesInOriginalOrder,
	...
	private final List<DiscreteVariable> order;
